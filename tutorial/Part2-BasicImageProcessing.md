...
Then, this image is converted into a Numpy array:
```python
#image_array is read-only so make a copy. Also, acts as a buffer for swapping pixel layers
...
Notice the *:* in the square brackets. With Numpy this basically means 'Take all the values in this dimension'. Another use is like this:

```python
somearray[10:]
```

This will use the values from index 10 to the end of the array.
...
output_image.save("processed_image-0.png")
```

This last step is easy but a little confusing. *np.uint8()* takes an array and casts it into a specific type. Because RGB colour space uses values between [0,255], the array is cast as a *uint8* (which is a byte with only positive values; [0,255]). This step is really only necessary if the processing step pushes the image array outside of this range (otherwise an error occurs)
